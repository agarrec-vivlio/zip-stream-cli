...
const { getRange } = require("../utils/rangeFetcher");
...
const getCentralDirectory = async (zipSize, url) => {
  try {
    ...
    if (eocdOffset === -1)
      throw new Error("Cannot find the End of Central Directory (EOCD) in the ZIP file.");
...
const listFiles = async (url) => {
  try {
    ...
    const cdirData = await getCentralDirectory(contentLength, url);
    const files = [];
